...
export default function About() {
  ...
  return (
    ...
    >
      <div
        className={`max-w-6xl mx-auto px-6 sm:px-8 py-28 transition-all duration-500 ease-out ${
          isIntersecting ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
        }`}
      >
        <h2 id="about-heading" className="font-heading text-h2 font-bold text-text-primary mb-6">
          {ABOUT.headline}
        </h2>
        <p className="text-lg text-text-secondary max-w-3xl mb-14 leading-relaxed">{ABOUT.body}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          {ABOUT.stats.map((stat) => (
            <div key={stat.label} className="border-l-2 border-accent/20 pl-6">
